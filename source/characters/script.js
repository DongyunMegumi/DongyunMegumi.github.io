...
(function () {
  ...
  async function init() {
    ...
    // 尝试加载已发布的数据（characters-data.json），以较新者为准
  ...
  function bindExportButton() {
    document.getElementById('exportDataBtn')?.addEventListener('click', async function () {
      ...
      showToast('已导出 characters-data.json — 上传到 GitHub 仓库 source/characters/ 目录即可发布', 'success');
    });
  }
  ...
  async function migrateFromLocalStorage() {
    try {
      const already = await localforage.getItem('cs_migrated');
      if (already) return;
      const raw = localStorage.getItem('cs_characters');
      if (raw) {
        await localforage.setItem('cs_characters', JSON.parse(raw));
      }
      await localforage.setItem('cs_migrated', true);
    } catch (e) { /* ignore */ }
  }
